...
//  })
  //Accessing application.conf file
  ...
  val financialAggregatorSink = Sink.foreach[Record](record =>{
    ...
    val fileContents = List(new SimpleDateFormat("yyyy").format(record.orderDate),record.category,formatValue(sales),quantity.toString,formatValue(discount),formatValue(profit))
  ...
  val bulkFinancialReportSink = Sink.foreach[Record](record =>{
    ...
    val fileContents = List(new SimpleDateFormat("yyyy").format(record.orderDate),formatValue(sales),formatValue(sales/count),quantity.toString,formatValue(quantity/count),formatValue(discount),formatValue(discount/count),formatValue(profit),formatValue(profit/count))
